...
import NavBarForTask from '../../components/NavBar/NavBarForTask'
...
class Tasks extends React.Component {
    ...
    static getDerivedStateFromProps(props, state) {
        const toko = localStorage.getItem('Token')
                                            ...
                                            {task.status !== 'COMPLETED' ?
                                                ...
                                                <div key={index} className="card text-white bg-dark mb-3" style={{ maxWidth: '18rem', margin: '10px', boxShadow:'0px 2px 30px 1px rgba(3,0,97,1)' }}>
                                                    <div className="card-header">
                                                        <p style={{ margin: '0', padding: '0', fontWeight: '900', color: 'yellow', display: 'inline' }}> {task.status} </p>
                                                        <i data-toggle="modal"
                                                            onClick={() => this.handleSomethingBig(task)}
                                                            data-target="#EditTaskModal"
                                                            style={{ float: 'right', cursor: 'pointer', color: 'lightblue', fontSize: 'larger' }}
